...
Out of sample error rates are measured by calculating **1 minus the Accuracy figure** that was calculated when validating the prediction model developed from the training dataset against the validation dataset. The Random Forest prediction model has a very high accuracy rate of **`r (100*(round(rfcm$overall['Accuracy'], 4)))`%**, so the out of sample error rate is **`r (100*(1-(round(rfcm$overall['Accuracy'], 4))))`%**. The Decision Tree prediction model has a lower accuracy rate of **`r (100*(round(dtcm$overall['Accuracy'], 4)))`%**, and therefore has a higher out of sample error rate of **`r (100*(1-(round(dtcm$overall['Accuracy'], 4))))`%**.
